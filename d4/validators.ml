...
let validate_eyr str = try Scanf.sscanf str "%i" (fun v -> in_range (2020, 2030) v) with Stdlib.Scanf.Scan_failure _ -> false
...
let validate_ecl str = try Scanf.sscanf str "%s" 
  (fun v ->  
      match v with
